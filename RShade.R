...
read_nhdplus_lines <- function(nhdplus_shapefile, nhdplus_id_fname) {
  message("Reading NHDPlus HR Shapefile")
  st_read(nhdplus_shapefile, quiet = TRUE) %>%
   st_zm() %>%
...
asp_all <- function(lines) {
  ...
  for (i in 1:nrow(lines)) {
    coords <- st_coordinates(lines$geometry[i])
...
get_river_lines <- function(gdb_path, nhdplus_lines) {
  streams_rivers <- st_read(gdb_path, layer = "NHDArea") %>%
    filter(FType == 460) %>%
    st_make_valid() %>%
    st_transform(5070)
...
#' boundary of NDH_Area, and measures the length. Returns the input data frame 
...
extract_horizon_angle <- function(points, hz_raster_list) {
