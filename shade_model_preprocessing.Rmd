...
# Whether the input format is a shapefile or a file geodatabase, the file MUST
# contain a variable named NHDPlusIDt (note: case-sensitive) that is a character
# version of NDHPlusID or COMID
# The default is to use flowlines from NHDPlus HR. If users wish to use an alternate
# flowline data source, set nonNHDPlusHR = TRUE. Example files for alternative
# flowlines are provided in the data directory as test2_nonNHDPlus.shp and
# test2_TotDASqKM.csv.

nonNHDPlusHR = FALSE

# replace "updates" with the name of the directory you created for this project

input_lines <- "~/private/updates/dmapSMP/data/test2.shp"
...
# Distances are in meters - these specify the vegetation zone widths
...
gets it into the correct coordinate system.
...
This optional step reads in an alternative source of watershed areas for users
employing a flowline not from NHDPlusHR.  The Excel or csv file must contain
the variables NHDPlusIDt and TotDASqKM (total drainage area in square km).  Edit
the block below to include the path to the excel or csv file.
...
```{r}
RShade_points2 <- add_latlon(RShade_points)
```
...
```{r}
RShade_points3 <- extract_elevation(RShade_points2, rpu_boundaries)
```
...
```{r}
RShade_points4 <- extract_horizon_angle(RShade_points3, rpu_boundaries)
```
...
```{r}
RShade_points5 <- star_sample(RShade_points4, 
                             veg_height, 
                             star_sample_angles, 
...
```{r}
RShade_points6 <- star_sample(RShade_points5, 
                             veg_cover, 
                             star_sample_angles, 
                             star_sample_distances, 
                             type="dens")
...
```{r}
RShade_points7 <- finalize_cols(RShade_points6, overhang_dividend)
```


Optional: If you have additional columns you want to add to the output, you can upload a
...
RShade_points7 <- copy_table(RShade_points7, nhdphr_lines, additional_table)
```
...
for RShade. 
```{r}
write_csv(RShade_points7, "data/Output.csv")
```
...
```{r}
st_write(RShade_points7, "data/Output.shp")
```
