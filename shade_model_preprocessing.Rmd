...
# as it will be ignored by the code. Sample points at which effective shade will
# be calculated will automatically be generated at x meter intervals, specified
# below by the user (default = 50m).
...
# Bankfull width (BFW) is calculated as a function of the watershed area using a
# coefficient (a) and an exponent (b) in the following equation:
# BFW = a * (WatershedArea ^ b)
# These values are determined automatically based on Hydrologic Landscape Region
# (HLR; Blackburn-Lynch et al. 2017, DOI:10.1111/1752-1688.12540). If you wish to use 
...
# Shade is calculated by examining vegetation heights and topographic heights
# (horizon angle) along transects radiating out from each shade point at
# distances defined by star_sample_distances
...
# The dividend (percentage) can be specified below. 
...
# file path "data/yourGrid.tif".  By default, vegetation cover and height are 
# derived from the USFS Landfire datasets (https://landfire.gov/) resampled to a 
# finer resolution
