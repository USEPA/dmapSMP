...
The Shade Model Preprocessing tool is a process that prepares NHDPlus HR flowline datasets for processing\
in RShade (available at https://github.com/USEPA/dmapSMP). It does this by taking the spatial data,\
calculating new columns, and exporting to a csv that can be input directly into RShade. For instructions\
on how to use this process, please see the shade_model_preprocessing.Rmd included in this repository.\

We suggest using an instance of 2xLarge (8 cpus, 32GB) for this program.  To Run Shade Model Preprocessing\
in an anote RStudio instance:\
...
- To update the code to the latest version, click the GIT icon below the menu bar (this looks like the\ 
word GIT turned 90-degrees). In the Github dropdown, select "Pull Branches. This will bring sync the most\
up to date version of code with your RStudio instance. 
...
from https://landfire.gov/vegetation, or substitute their own datasets at a finer resolution.\
...
projected in NAD Conus Albers (EPSG:5070), and placed in the appropriate subdirectories under data:\
...
Horizon angle grids (e.g., 1a.tif, named after RPU zone containing network) - horizon angle grids.\
In this version, the raster must be organized as follows:\
...
Vegetation height raster (meters,LC22_EVH_220_reclass.tif) – see Landfire metadata for description\
(https://landfire.gov/sites/default/files/metadata/LF2025/CONUS/LF2025_EVC_CONUS.xml)

Vegetation density raster (percent cover,LC22_ECH_220_reclass.tif) – see Landfire metadata for description\
(https://landfire.gov/sites/default/files/metadata/LF2025/CONUS/LF2025_EVH_CONUS.xml)

Elevation (e.g., 1a.tif, named after RPU zone containing network)  - digital elevation model grid (meters)\
